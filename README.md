...
- An **explicit opt-in** Phase B native `OverlayAction` renderer core (`source.create` kind `"native-overlay"`; the default browser-source `source.create` path is untouched). A plain host-repo library (`src/native_overlay_renderer.*`) rasterizes the eight budgeted Spec 07 parity categories (`toast`, `lower-third`, `vote-callout`, `vote-result`, `celebration-burst`, `generated-image`, `sound-cue`, `clear`) at 1280×720 into a CoreGraphics `CGBitmapContext`-backed surface, with apply→rendered timing keyed to each category's Spec 07 budget keys. This is a renderer *core*, not a graduation: ADR-0003's browser-source default stands, Q-33 stays open, and nothing here is claimed as promoted or default.
